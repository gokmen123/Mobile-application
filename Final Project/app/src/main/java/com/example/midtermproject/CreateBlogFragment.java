...
//        int leng = firestore.collection("Users").document(username).get().getResult().getString("userBlog").length();
        ...
        create.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Blog blog = new Blog(0,0,username,newDate,thoughts.getText().toString(),
                        title.getText().toString(),username);
                firestore.collection("Users").document(username).update("userBlog", FieldValue.arrayUnion(blog)).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
